...
int main() {
    for (int i = 0; i < NUM_TRIALS; i++) {
        int num = randomInteger(-100, 100);
        int base = randomInteger(2, 36);
        string sub = getSubscript(base);
        string convertStr = integerToString(num, base);
        int convertNum = stringToInteger(convertStr, base);
        cout << setw(15) << "integerToString: " << num << "\u2081\u2080" << " = " << convertStr << sub << endl;
        cout << setw(15) << "stringToInteger: " << convertStr << sub << " = " << convertNum << "\u2081\u2080" << endl;
        cout << endl;
    }
