 ...
 *  might encounter in a grocery store. Customers arrive at the
 *  checkout stand and get in line. Those customers arrive wait
 *  in the line until the cashier is free, at which point they are
 *  served and occupy the cashier for some periodof time.
 *  After the service time is complete, the cashier is free
 *  to serve the next custormer in the line.
 *
 *  In each unit of time, up to the constant SIMULATION_TIME, the
 *  following operations are performed:
 *
 *  1. Determine whether a new customer has arrived. 
 *  	New customers arrive randomly, with a probability 
 *  	determined by the constant ARRRIVAL_PROBABILITY.
 *  2. If the cashier is busy, note that the cashier has
 *  	spent another minute with that customer. Eventually, 
 *  	the customer's time request is satisfied, which frees the
 *  	cashier.
 *
 *  3.If the cashier is free, serve the next customer in line.
 *   The service time is taken to be a random period between MIN_SERVICE_TIME
 *   and MAX_SERVICE_TIME
 *
 *   At the end of the simulation, the program displays the simulation constants
 *   and the following results:
 *
 *   - The number of custormers served
 ...
 * Usage: runSimulation();
 * ------------------------
 *  Runs the actual simulation. This function returns the result of
 *  the simulation through the reference parameters, which record the
 *  number of customers served, the total number of seconds that 
 *  customer were waiting in a queue, and the sum of the queue length
 *  in each time step.
 */
