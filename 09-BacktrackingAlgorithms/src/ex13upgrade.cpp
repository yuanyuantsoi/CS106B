 ...
 */
    void makeMove(moveT move) {
        nCoins -= move.nTaken;
        if (move.targetPlayer == HUMAN) nCoinsHu += move.nTaken;
 ...
 */
    ...
    moveT findBestMove(int depth, int & rating) {
        ...
        for (moveT move : moveList) {
            move.targetPlayer = (isEven(depth)) ? COMPUTER : HUMAN;
            makeMove(move);
 ...
 */
    void retractMove(moveT move) {
        nCoins += move.nTaken;
        if (move.targetPlayer == COMPUTER) nCoinsCom -= move.nTaken;
